...
import {MarketSmartContract, Offer} from '../sdk/marketSmartContract.sdk'
...
export const Market = () => {
  ...
  const [fullOffers, setFullOffers] = useState<{Offer: Offer; Car: Car}[]>()
    ...
    Promise.all([
      ...
      .then(([market, minter, offers]) => {
        ...
        Promise.all(
          offers.map(
            (offer: Offer) =>
              Car.fromApi(offer.carTokenIdentifier!, offer.carNonce).then(
                (car) => ({Offer: offer, Car: car})
              ) // add the Car object to the Offer object
          )
        ).then((updatedOffers) => {
      ...
      {fullOffers?.map((offer) => (
        <Card className="p-2 m-4">
          <CardBody>
            ...
            <div>
              Odometer Values: {offer.Car.historyData.odometerValues.join(', ')}
            </div>
            <div>
              Odometer Timestamps:{' '}
            ...
            <div>Car Status: {offer.Offer.status} </div>
          </CardBody>
          <Divider />
          <CardFooter>
